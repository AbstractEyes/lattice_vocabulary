...
def generate_fusion_stubs():
    ...
    if not fusion_init.exists():
        print("⚠ Fusion __init__.py not found, skipping")
        return None
    ...
    # Read existing __init__.py to preserve manual exports
    init_content = fusion_init.read_text()

    # Copy import statements exactly as they are
    for line in init_content.split('\n'):
        stripped = line.strip()
        if stripped.startswith('from .') or stripped.startswith('import'):
            lines.append(line + '\n')

    # Add __all__ type hint if it exists
...
def main():
    ...
    try:
        ...
        print("\nRestart PyCharm or run 'File → Invalidate Caches' for changes to take effect if changes aren't immediate.")
